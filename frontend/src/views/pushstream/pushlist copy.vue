...
import { ipcApiRoute,videoApi} from '@/api/main';
...
const list = ref([])
const fetchList = ()=> {
  ipc.invoke(ipcApiRoute.list,'').then(response => {
    list.value = response
  })
}
fetchList()
...
const handleChangeEnabled = (value)=>{
  ipc.invoke(ipcApiRoute.enabled,toRaw(value)).then(res=>{
...
const handleDelete = (id)=> {
  ...
  ipc.invoke(ipcApiRoute.delete,id).then(res=>{
    fetchList()
  })
}
